...
#     new_data = nd.zeros((n, 224, 224, data.shape[3])
#     for i in range(len(data)):
#         new_data[i] = mx.image.imresize(data[i], 224, 224)
#     data = new_data
#
#     return nd.transpose(data.astype('float32'), (2, 0, 1)) / 255, label.astype('float32')

train_data = transform_mnist(X_train, Y_train)
test_data = transform_mnist(X_test, Y_test)




batch_size = 64
train_data = mx.gluon.data.DataLoader(train_data, batch_size = batch_size, shuffle=False, last_batch = 'discard')

test_data = mx.gluon.data.DataLoader(test_data, batch_size = batch_size, shuffle=False, last_batch = 'discard')
